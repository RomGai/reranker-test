...
class DistillSFTTrainer(SFTTrainer):
    ...
    def _load_teacher_logits(self, batch_size: int, it: int, dp_rank: int, device: torch.device, no_model_batch: Dict):
        # 正确的多卡索引计算
        # global_idx = (it * torch.distributed.get_world_size() + dp_rank) * batch_size
        # loaded_data = self.teacher_logits[global_idx : global_idx + batch_size]
...
def train(config):
    ...
    # print(" bf16 =", training_arguments.bf16)
    # print(" fp16 =", training_arguments.fp16)
    # print(" tf32 =", training_arguments.tf32)
    # print(" torch_dtype =", getattr(training_arguments, 'torch_dtype', None))
